...
public class AlarmRestServiceTest extends AbstractSpringJerseyRestTestCase {
    ...
    public void testAlarmsJson() throws Exception {
        ...
        // GET all items
        MockHttpServletRequest jsonRequest = createRequest(getServletContext(), GET, url, "admin", Arrays.asList(new String[]{ Authentication.ROLE_ADMIN }));
    ...
    public void testAlarmUpdates() throws Exception {
        ...
        sendPut("/alarms/" + alarmId, "ticketId=12345", 303, "/alarms/" + alarmId);
        xml = sendRequest(GET, "/alarms/" + alarmId, 200);
        assertTrue(xml.contains("<troubleTicket>12345</troubleTicket>"));

        sendPut("/alarms/" + alarmId, "ticketState=UPDATE_PENDING", 303, "/alarms/" + alarmId);
        xml = sendRequest(GET, "/alarms/" + alarmId, 200);
        sendPut("/alarms/" + alarmId, "ticketState=UPDATE_PENDING", 303, "/alarms/" + alarmId);
